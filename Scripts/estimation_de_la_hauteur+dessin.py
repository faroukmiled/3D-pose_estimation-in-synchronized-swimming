        ...
        if int(row[0])==e:
            ...
            if int(row[0])>=240:
                    ...
                    if b<d and a!=0 and b!=0 :# figure and 
                        ...
                        text=str(round(distance_euclidienne(point1,point2),2))
        
                        # Tracé de la ligne entre les deux points
                        draw.line([point1, point2], fill="red", width=6)
                        font = ImageFont.truetype('arial.ttf', 36)
                        draw.text(((a+c)//2,(b+d)//2), text, font=font, align='right')
        
                        # Enregistrement de l'image modifiée
                        im.save("output_hauteur_bis/frame"+str(e)+"_height_estimate.jpg")
                    L=[]
                    e=int(row[0])
        count+=1
